...
import {
  ...
  cutValue, DEFAULT_CIM, GRAPH_INFO, BRUTE_FORCE_MAX_N,
...
export default function IsingLab() {
  const [kind, setKind] = useState<GraphKind>('cubic');
  ...
  const graph = useMemo(() => makeGraph(kind, n, seed), [kind, n, seed]);
    ...
    <div className="grid grid-cols-1 gap-3 xl:grid-cols-[minmax(0,1fr)_310px]">
      <div className="space-y-3">
        ...
        <Panel
          ...
          right={<Btn tone="primary" onClick={solve} disabled={busy}>{busy ? '⋯ calcul' : '▶ Résoudre'}</Btn>}
          flush
        >
          {!bench ? (
      ...
      {/* controls */}
      <div className="space-y-3">
        <Panel tag="GRAPH" title="Instance" subtitle={GRAPH_INFO[kind]}>
