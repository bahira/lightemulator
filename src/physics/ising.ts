...
/**
 * Parse une matrice de couplage collée par l'utilisateur (CSV / espaces / ;).
 * Symétrisée par moyenne, diagonale ignorée, poids nuls → pas d'arête.
 * Retourne null si le format est invalide (taille, valeurs non numériques).
 */
export function parseWeightMatrix(text: string, name = 'custom'): Graph | null {
  ...
  for (const r of rows) {
    ...
    for (const v of r) if (!Number.isFinite(v)) return null;
  }
  const eu: number[] = [], ev: number[] = [], ew: number[] = [];
  for (let i = 0; i < n; i++) {
    for (let j = i + 1; j < n; j++) {
      const w = (rows[i][j] + rows[j][i]) / 2;
      if (Math.abs(w) > 1e-9) { eu.push(i); ev.push(j); ew.push(w); }
    }
  }
  const deg = new Int32Array(n);
  for (let k = 0; k < eu.length; k++) { deg[eu[k]]++; deg[ev[k]]++; }
  const off = new Int32Array(n + 1);
  for (let i = 0; i < n; i++) off[i + 1] = off[i] + deg[i];
  const adj = new Int32Array(2 * eu.length), aw = new Float32Array(2 * eu.length);
  const cur = Int32Array.from(off.subarray(0, n));
  for (let k = 0; k < eu.length; k++) {
    adj[cur[eu[k]]] = ev[k]; aw[cur[eu[k]]++] = ew[k];
    adj[cur[ev[k]]] = eu[k]; aw[cur[ev[k]]++] = ew[k];
  }
  let totalW = 0;
  for (const w of ew) totalW += w;
  return {
    n, m: eu.length,
    eu: Int32Array.from(eu), ev: Int32Array.from(ev), ew: Float32Array.from(ew),
    off, adj, aw, totalW,
