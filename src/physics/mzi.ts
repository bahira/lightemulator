...
// ---------------------------------------------------------------------------
//  Import d'une matrice collée par l'utilisateur — réels ou complexes "a+bi"
// ---------------------------------------------------------------------------

const NUM = '[+-]?(?:\\d+\\.?\\d*|\\.\\d+)(?:[eE][+-]?\\d+)?';

/** Token complexe : "1", "-0.5", "i", "2i", "1+2i", "0.3-0.4j". */
export function parseComplexToken(tok: string): [number, number] | null {
  const s = tok.trim().toLowerCase().replace(/j/g, 'i');
  if (!s) return null;
  let m = s.match(new RegExp(`^(${NUM})$`));
  if (m) return [parseFloat(m[1]), 0];
  m = s.match(new RegExp(`^(${NUM}?)i$`));
  if (m) {
    const c = m[1];
    return [0, c === '' || c === '+' ? 1 : c === '-' ? -1 : parseFloat(c)];
  }
  m = s.match(new RegExp(`^(${NUM})(${NUM})i$`));
  if (m && /^[+-]/.test(m[2])) return [parseFloat(m[1]), parseFloat(m[2])];
  return null;
}

/**
 * Parse une matrice carrée collée (CSV / espaces / ;), réelle ou complexe.
 * Retourne null si non carrée, vide, ou token illisible. n ∈ [2, 40]
 * (décomposition O(n³) — au-delà l'UI gèle inutilement).
 */
export function parseMatrixText(text: string): CMat | null {
  ...
  for (let i = 0; i < n; i++) {
    if (rows[i].length !== n) return null;
    for (let j = 0; j < n; j++) {
      const c = parseComplexToken(rows[i][j]);
      if (!c) return null;
      re[i * n + j] = c[0]; im[i * n + j] = c[1];
    }
  }
  return { n, re, im };
}
