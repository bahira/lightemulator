...
import { Panel, Stat, Btn, Tag, downloadFile } from '../ui/kit';
...
export default function ValidationLab() {
  ...
  // rapport complet en Markdown — falsifiable, archivable, diffable
  const exportReport = useCallback(() => {
    const allNow = TEST_ORDER.map((id) => results[id]).filter(Boolean) as TestResult[];
    if (allNow.length === 0) return;
    const lines = [
      `# Rapport grounded — ${allNow.length}/${TEST_ORDER.length} tests`,
      `Généré : ${new Date().toISOString()} · navigateur : ${navigator.userAgent}`,
      '',
      '| Groupe | Test | Résidu | Tolérance | Statut |',
      '|---|---|---|---|---|',
      ...allNow.map((r) =>
        `| ${r.group} | ${r.name} | ${formatResidual(r)} | ${r.tolerance} | ${r.pass ? '✅' : '❌'} |`),
      '',
      '## Références indépendantes',
      ...allNow.map((r) => `- **${r.name}** — ${r.claim}. Réf : ${r.reference}${r.extra ? `. ${r.extra}` : ''}`),
    ];
    downloadFile('rapport_grounded.md', lines.join('\n'), 'text/markdown');
  }, [results]);
      ...
      <Panel
        ...
        right={
          <div className="flex gap-1.5">
            <Btn onClick={exportReport} disabled={all.length === 0} title="télécharger le rapport Markdown">↓ Rapport</Btn>
            <Btn tone="primary" onClick={runAll} disabled={queue.length > 0}>{queue.length > 0 ? '⋯ exécution' : '↻ Tout relancer'}</Btn>
          </div>
        }
      >
